...
async function main() {
    ...
    const deposit_data = JSON.parse(fs.readFileSync("./validator_keys2/deposit_data-1658734582.json", "utf-8"));
    ...
    for (const elem of deposit_data) {
        ...
        await contract
            ...
            .deposit_with_voter(prefix0X(elem.pubkey), prefix0X(elem.withdrawal_credentials), prefix0X(elem.signature), prefix0X(elem.deposit_data_root),
                voter_info,{
                    from: admin.address,
                    value: TX_VALUE,
                });
